...
import ListaMovimientos from './L_movimientos.jsx'
...
const Dashboard = ({ usuarioActual, onLogout }) => {
  ...
  const renderContenido = () => {
    switch (tabActiva) {
      case 'movimientos': return <ListaMovimientos />
      ...
      case 'proveedores': return <ListaProveedores esAdmin={esAdmin} />
  ...
  return (
    <div>
      <div className="card shadow text-center p-4 mb-4">
        <h1 className="mb-2">Gestión Ferretería</h1>
      ...
      <nav className="card shadow p-3 mb-4">
        <div className="d-flex gap-2 flex-wrap">
          <button 
            className={`btn ${tabActiva === 'movimientos' ? 'btn-warning' : 'btn-outline-secondary'}`} 
          ...
          <button 
            className={`btn ${tabActiva === 'proveedores' ? 'btn-warning' : 'btn-outline-secondary'}`} 
      ...
      <div className="mt-5 pt-3">
        <button className="btn btn-danger px-4 py-2" onClick={onLogout}>Cerrar Sesión</button>
      </div>
    </div>
  )
}
