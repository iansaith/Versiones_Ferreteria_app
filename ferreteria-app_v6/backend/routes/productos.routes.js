  ...
  try {
    const [rows] = await pool.query(
      `SELECT 
       ...
       GROUP BY p.idProducto, p.Nombre_Producto, p.Stock_Minimo, p.ubicacion,
                p.price, p.wholesalePrice, p.profit,
                c.Nombre_Categoria, c.idCategoria, pr.Nombre_Proveedor, pr.idProveedor`
    )
    res.json(rows)
  } catch (error) {
    console.error(error)
